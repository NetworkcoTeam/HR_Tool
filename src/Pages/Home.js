...
const Home = () => {
 ...
 const handleAddTaskClick = () => {
  ...
  const fetchLeaveRequests = async (empId) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/LeaveRequest/employee/${empId}`, {
    ...
    <Layout style={{ minHeight: '100vh' }}>
      ...
      <Layout className="site-layout">
        <Content className="home-content">
          ...
          <Modal title="Payslip Details" visible={isModalVisible} onCancel={handleModalCancel} footer={<Button onClick={handleModalCancel}>Close</Button>}>
            {currentPayslipDetails ? (
              <div>
                <p><strong>Employee Name:</strong> {currentPayslipDetails.employeeName}</p>
                <p><strong>Position:</strong> {currentPayslipDetails.position}</p>
                ...
                <p><strong>Generated Date:</strong> {currentPayslipDetails.generatedDate}</p>
              </div>
