...
const Leave = () => {
  ...
  const fetchLeaveRequests = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/employee/${userData.employee_id}`, {
          ...
          <div className="leave-requests-container">
            ...
            >
              ...
              >
                <Row gutter={24}>
                  <Col xs={24} md={12}>
                    <Form.Item name="name" label="Name" rules={[{ required: true, message: 'Please input your name!' }]}>
                      <Input prefix={<UserOutlined />} placeholder="Enter your name" disabled={!!userData?.name} />
                    </Form.Item>

                    <Form.Item name="employeeId" label="Employee ID" rules={[{ required: true, message: 'Please input your employee ID!' }]}>
                      <Input prefix={<IdcardOutlined />} placeholder="Enter employee ID" disabled={!!userData?.employee_id} />
                    </Form.Item>

                    <Form.Item name="position" label="Position" rules={[{ required: true, message: 'Please input your position!' }]}>
                      <Input placeholder="Enter your position" />
                    </Form.Item>

                    <Form.Item name="leaveStart" label="Leave Start Date" rules={[{ required: true, message: 'Please select start date!' }]}>
                  ...
                  <Col xs={24} md={12}>
                    <Form.Item name="surname" label="Surname" rules={[{ required: true, message: 'Please input your surname!' }]}>
                      <Input placeholder="Enter your surname" disabled={!!userData?.surname} />
                    </Form.Item>
                    ...
                    <Form.Item name="leaveEnd" label="Leave End Date" rules={[{ required: true, message: 'Please select end date!' }]}>
                      <DatePicker style={{ width: '100%' }} suffixIcon={<CalendarOutlined />} />
                    </Form.Item>
                ...
                <Form.Item name="leaveType" label="Type of Leave" rules={[{ required: true, message: 'Please select leave type!' }]}>
                  <Radio.Group 
                    options={leaveTypes} 
                ...
                {leaveType === 'other' && (
                  <Form.Item name="otherLeaveType" label="Please specify leave type" rules={[{ required: true, message: 'Please specify leave type!' }]}>
                ...
                <Form.Item name="doctorsLetter" label="Doctor's Letter (if applicable)" valuePropName="fileList" getValueFromEvent={normFile}>
                  <Upload beforeUpload={() => false} listType="text" maxCount={1}>
                    <Button icon={<UploadOutlined />}>Click to upload</Button>
                  </Upload>
                </Form.Item>

                <Form.Item name="funeralLetter" label="Funeral Letter (if applicable)" valuePropName="fileList" getValueFromEvent={normFile}>
                  <Upload beforeUpload={() => false} listType="text" maxCount={1}>
                    <Button icon={<UploadOutlined />}>Click to upload</Button>
                  </Upload>
                </Form.Item>
